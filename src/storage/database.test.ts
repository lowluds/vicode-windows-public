...
function setRunEventsCreatedAt(
  db: DatabaseService,
  threadId: string,
  runId: string,
  createdAt: string,
  ...
  rawDatabase(db)
    .prepare(
      "UPDATE run_events SET created_at = ? WHERE thread_id = ? AND run_id = ?",
    )
    .run(createdAt, threadId, runId);
}

describe("DatabaseService bootstrap defaults", () => {
  it("starts fresh installs without a seeded placeholder project", async () => {
    ...
    expect(db.getPreferences().selectedProjectId).toBeNull();
  });

  it("removes the legacy folderless placeholder project during migration", async () => {
    const dir = await mkdtemp(join(tmpdir(), "vicode-storage-"));
    cleanupPaths.push(dir);
    ...
    legacyDb.migrate();
    const placeholder = legacyDb.createProject({
      name: "My Project",
      trusted: false,
    });
    legacyDb.close();
    ...
    expect(reopened.getPreferences().selectedProjectId).toBeNull();
    expect(() => reopened.getProject(placeholder.id)).toThrow();
  });
});
...
describe("DatabaseService storage diagnostics", () => {
  ...
  it("persists MCP server scope, project ownership, and lifecycle state", async () => {
    ...
    const listed = db
      .listMcpServers()
      .find((entry) => entry.definition.id === server.definition.id);
  ...
  it("compacts delta events only for old archived terminal runs", async () => {
    ...
    db.addRunEvent(thread.id, "run-compact", "started", {
      providerId: "openai",
    });
  ...
  it("does not compact recent runs, planner-active runs, or runs with pending review work", async () => {
    ...
    db.addRunEvent(recentThread.id, "run-recent", "completed", {
      output: "done",
    });
    ...
    db.addRunEvent(pendingReviewThread.id, "run-review", "delta", {
      delta: "c",
    });
    db.addRunEvent(pendingReviewThread.id, "run-review", "completed", {
      output: "done",
    });
    db.archiveThread(pendingReviewThread.id);
    setRunEventsCreatedAt(
      db,
      pendingReviewThread.id,
      "run-review",
      "2026-01-01T00:00:00.000Z",
    );
    ...
    expect(
      db.getThread(recentThread.id).rawOutput.map((event) => event.eventType),
    ).toEqual(["started", "delta", "completed"]);
    expect(
      db.getThread(plannerThread.id).rawOutput.map((event) => event.eventType),
    ).toEqual(["started", "delta", "completed"]);
    expect(
      db
        .getThread(pendingReviewThread.id)
        .rawOutput.map((event) => event.eventType),
    ).toEqual(["started", "delta", "completed"]);
  });
  ...
  it("preserves info and terminal events when compacting an old archived failed run", async () => {
    ...
    db.addRunEvent(thread.id, "run-failed", "started", {
      providerId: "openai",
    });
    ...
    db.addRunEvent(thread.id, "run-failed", "failed", {
      message: "planner disconnected",
    });
    db.archiveThread(thread.id);
    ...
    expect(detail.rawOutput.map((event) => event.eventType)).toEqual([
      "started",
      "info",
      "failed",
    ]);
  ...
  it("recovers interrupted active threads on startup by marking them aborted", async () => {
    ...
    db.addRunEvent(thread.id, "run-recover", "delta", {
    ...
    expect(updated.rawOutput.map((event) => event.eventType)).toEqual([
      "started",
      "delta",
      "aborted",
    ]);
  ...
  it("supersedes older queued steer items for the same target run and lists remaining queued threads", async () => {
    ...
    expect(db.listThreadFollowUps(thread.id).map((item) => item.id)).toEqual([
      newer.id,
    ]);
...
describe("DatabaseService generated memory storage", () => {
  it("dedupes generated-memory candidates by workspace scope and dedupe key", async () => {
    ...
    const turn = db.appendTurn(
      thread.id,
      "user",
      "Use npm run smoke when runtime orchestration changes.",
    );
    ...
    const first = db.upsertGeneratedMemoryCandidate({
      ...
      detail:
        "Prefer npm run smoke when main-process runtime orchestration changes.",
    ...
    const second = db.upsertGeneratedMemoryCandidate({
      ...
      detail:
        "Prefer npm run smoke before trusting runtime orchestration changes.",
      evidenceExcerpt: "Use npm run smoke when runtime orchestration changes.",
      dedupeKey: `${project.folderPath}|workflow_preference|npm-run-smoke`,
      status: "consolidated",
      createdAt: "2026-04-20T10:00:00.000Z",
  ...
  it("stores generated-memory items with evidence and supports disabling recall", async () => {
    ...
    const turn = db.appendTurn(
      thread.id,
      "assistant",
      "Avoid the legacy path; use the workspace root script.",
    );
    const candidate = db.upsertGeneratedMemoryCandidate({
      ...
      detail:
        "Use the workspace root script instead of the legacy nested path.",
    ...
    const disabled = db.disableGeneratedMemoryItem(
      item.id,
  ...
  it("clears generated-memory state per workspace scope without touching another workspace", async () => {
    ...
    const turnA = db.appendTurn(
      threadA.id,
      "user",
      "Workspace A prefers pnpm.",
    );
    ...
    expect(db.listGeneratedMemoryCandidates(projectA.folderPath!)).toHaveLength(
      0,
    );
    expect(db.listGeneratedMemoryItems(projectA.folderPath!)).toHaveLength(0);
    expect(db.listGeneratedMemoryCandidates(projectB.folderPath!)).toHaveLength(
      1,
    );
...
describe("DatabaseService collaboration cache", () => {
  it("hydrates collaboration bootstrap from local cache tables", async () => {
    ...
    expect(bootstrap.messagesByRoom["room-1"][0]?.body).toBe(
      "Shared run is live.",
    );
    expect(bootstrap.presenceByRoom["room-1"][0]?.activeRunId).toBe("run-1");
    expect(bootstrap.sharedThreadsByRoom["room-1"][0]?.title).toBe(
      "Collaboration bootstrap",
    );
    expect(bootstrap.sharedRunsByRoom["room-1"][0]?.status).toBe("running");
    expect(bootstrap.handoffsByRoom["room-1"][0]?.title).toBe(
      "Continue backend wiring",
    );
    expect(bootstrap.contacts.map((contact) => contact.userId)).toEqual([
      "user-2",
    ]);
  });
  ...
  it("includes direct-message rooms in chats while preserving contact discovery", async () => {
    ...
    expect(bootstrap.messagesByRoom["dm-1"][0]?.body).toBe(
      "Ping from the peer",
    );
    expect(bootstrap.contacts.map((contact) => contact.userId)).toContain(
      "user-2",
    );
  });
});
