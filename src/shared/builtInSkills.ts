...
export const builtInSkillSeeds: BuiltInSeed[] = [
  ...
  },
  {
    ...
    instructions:
      'When the user wants a new Vicode skill, prefer the native `create_skill_bundle` tool when it is available so the final bundle lands in Vicode app state without using workspace file tools for app-owned folders. Prefer one folder per skill. Include SKILL.md with frontmatter for name and description, then concise operational instructions in the body. Add .vicode-skill.json only when the request needs non-default metadata such as slug, scope, providerTargets, syncTargets, enabled state, or category. Keep the instructions tight, practical, and easy for another agent to follow. Fall back to direct folder writes only when the request explicitly points at a different writable folder outside the app-managed creator flow. When finished, summarize the folder name, intended use, and any non-default metadata you chose.',
    providerTargets,
    metadata: {
      slug: 'skill-creator',
      category: 'templates',
