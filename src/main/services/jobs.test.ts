...
class ProviderManagerStub {
  ...
  submitComposer = vi.fn(
    async (input: { projectId: string; threadId?: string }) => {
...
describe("JobsService", () => {
  ...
  it("restores the job to waiting_for_review if provider startup fails during approval", async () => {
    ...
    await expect(jobs.approveReview(queued.reviewItem.id)).rejects.toThrow(
  ...
  it("blocks duplicate queueing while an automation job is already active", async () => {
    ...
    await expect(
      jobs.enqueueAutomationJob(automation.id, "manual"),
  ...
  it("writes a daily note immediately without starting a provider run", async () => {
    ...
    db.appendTurn(
      thread.id,
      "user",
      "Please keep today's focus in the daily note.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "I will draft that memory update for review.",
    );

    const providers = new ProviderManagerStub();
    const memoryWrites = new MemoryWritesService(
      db,
      new WorkspaceMemoryService(db),
    );
    ...
    expect(String(applied.reviewItem.decision?.writtenPath ?? "")).toContain(
      "memory",
    );
    expect(
      existsSync(String(applied.reviewItem.decision?.writtenPath ?? "")),
    ).toBe(true);
    expect(
      readFileSync(
        String(applied.reviewItem.decision?.writtenPath ?? ""),
        "utf8",
      ),
    ).toContain("Capture daily memory");
  ...
  it("writes a MEMORY.md promotion immediately without starting a provider run", async () => {
    ...
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-memory-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "user",
      "The durable workspace files are now the canonical source of truth.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "That should be reviewed and promoted into MEMORY.md.",
    );
    ...
    ).toContain("canonical source of truth");
  ...
  it("writes USER.md immediately without starting a provider run", async () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-user-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "user",
      "Please keep answers concise and explain tradeoffs when the change is risky.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "That sounds like a USER.md candidate.",
    );
  ...
  it("silently reconciles stale pending memory review items on startup", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-reconcile-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    const jobs = new JobsService(
      db,
      providers as never,
      new MemoryWritesService(db, new WorkspaceMemoryService(db)),
    );
    ...
    expect(readFileSync(join(workspaceDir, "USER.md"), "utf8")).toContain(
      "Keep answers concise.",
    );
  ...
  it("updates a pending manual review draft before approval", async () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-edit-review-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "user",
      "Capture the current review workflow in the daily note.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "I will prepare a daily note draft for review.",
    );
    ...
    expect(String(approved.reviewItem.decision?.writtenPath ?? "")).toContain(
      "memory",
    );
    expect(
      readFileSync(
        String(approved.reviewItem.decision?.writtenPath ?? ""),
        "utf8",
      ),
    ).toContain("Keep this custom draft instead of the generated one.");
  ...
  it("starts a future-system heartbeat task as a delegated background run", async () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-heartbeat-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
  ...
  it("auto-applies a daily note update after a meaningful completed run on a trusted workspace", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-auto-daily-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "assistant",
      "I inspected the repo and started the feature shell.",
    );
    db.appendTurn(
      thread.id,
      "user",
      "Now refine the data flow and verify the UI states.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "The core flow is working and the verification pass is complete.",
    );
    ...
    const dailyJobs = db
      .listJobs()
      .filter(
        (job) => job.sourceId === `daily-note:${thread.id}:run-auto-daily`,
      );
  ...
  it("does not silently auto-apply MEMORY.md or USER.md even when the thread contains durable signals", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-auto-memory-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "assistant",
      "Understood. I will keep answers concise and explain tradeoffs when the change is risky.",
    );
    db.appendTurn(
      thread.id,
      "user",
      "Remember this: the workspace files are the canonical source of truth and that workflow should remain the default.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "I will remember that the workspace files are the canonical source of truth and keep that workflow as the default.",
    );
    ...
    expect(
      db
        .listJobs()
        .filter(
          (job) => job.threadId === thread.id && job.status === "completed",
        )
  ...
  it("does not auto-apply MEMORY.md or USER.md from a single early prompt with no lifecycle pressure", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-early-prompt-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "user",
      "Please keep answers concise and treat the workspace files as the canonical source of truth.",
    );
    ...
    expect(existsSync(join(workspaceDir, "USER.md"))).toBe(false);
    expect(
      db.listJobs().filter((job) => job.threadId === thread.id),
    ).toHaveLength(0);
  ...
  it("captures generated-memory candidates in shadow mode after a meaningful completed run", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-generated-memory-workspace-"),
    );
    const artifactsRoot = mkdtempSync(
      join(tmpdir(), "vicode-jobs-generated-memory-artifacts-"),
    );
    tempDirs.push(workspaceDir);
    ...
    db.appendTurn(
      thread.id,
      "assistant",
      "Use npm run smoke from the workspace root instead of the legacy nested path.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
    ...
    const workspaceScopeKey =
      normalizeGeneratedMemoryWorkspaceScopeKey(workspaceDir);
    const artifactDir = getGeneratedMemoryScopeArtifactDir(
      artifactsRoot,
      workspaceScopeKey,
    );
    ...
    expect(
      candidates.every((candidate) => candidate.status === "consolidated"),
    ).toBe(true);
    ...
    expect(
      readFileSync(join(artifactDir, "generated-memory-summary.md"), "utf8"),
    ).toContain("shadow mode active");
    expect(
      readFileSync(join(artifactDir, "generated-memory-summary.md"), "utf8"),
    ).toContain("Recallable consolidated items: 3");
    expect(
      readFileSync(join(artifactDir, "generated-memory-items.md"), "utf8"),
    ).toContain("derived_noncanonical");
    expect(
      readFileSync(
        join(artifactDir, "generated-memory-evidence", `${thread.id}.md`),
        "utf8",
      ),
    ).toContain("Artifact subject: consolidated item");
    expect(
      readFileSync(
        join(artifactDir, "generated-memory-evidence", `${thread.id}.md`),
        "utf8",
      ),
    ).toContain("legacy nested path");
    expect(
      readFileSync(
        join(artifactDir, "generated-memory-evidence", `${thread.id}.md`),
        "utf8",
      ),
    ).toContain("default verification command");
    expect(db.listPendingReviewItems()).toHaveLength(0);
    expect(existsSync(join(workspaceDir, "MEMORY.md"))).toBe(false);
    expect(existsSync(join(workspaceDir, "USER.md"))).toBe(false);
  ...
  it("skips generated-memory capture when generated-memory generation is disabled in preferences", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-generated-memory-disabled-"),
    );
    const artifactsRoot = mkdtempSync(
      join(tmpdir(), "vicode-jobs-generated-memory-disabled-artifacts-"),
    );
  ...
  it("auto-applies a daily note update when a completed run finishes under high context pressure", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-context-pressure-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    expect(
      db
        ...
        .some(
          (job) =>
            job.sourceId === `daily-note:${thread.id}:run-context-pressure` &&
  ...
  it("flushes a silent daily note before completion when context pressure reaches compaction risk", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-pre-compaction-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
    ...
    db.appendTurn(
      thread.id,
      "user",
      "Continue the implementation and keep the working contract intact.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "I am continuing the implementation and keeping the working contract intact.",
    );
    ...
    expect(
      db
        ...
        .some(
          (job) =>
            job.sourceId === `daily-note:${thread.id}:run-pre-compaction` &&
            job.status === "completed",
        ),
    ).toBe(true);
    expect(
    ...
    ).toContain("Pre-compaction memory flush thread");
  ...
  it("does not auto-queue durable reviews for untrusted workspaces even when the thread contains strong signals", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-untrusted-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
    ...
    db.appendTurn(
      thread.id,
      "user",
      "Please keep answers concise and treat MEMORY.md as canonical.",
    );
    db.appendTurn(
      thread.id,
      "assistant",
      "That should stay the canonical source of truth.",
    );
  ...
  it("does not auto-apply duplicate daily note writes when the same completed signal is emitted twice", () => {
    const db = createDb();
    const workspaceDir = mkdtempSync(
      join(tmpdir(), "vicode-jobs-duplicate-completed-workspace-"),
    );
    tempDirs.push(workspaceDir);
    const project = db.createProject({
      name: "Trusted workspace",
