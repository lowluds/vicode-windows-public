...
export function LandingBeams() {
  ...
  useEffect(() => {
    ...
    const fragSrc = `
      ...
      void main() {
        vec2 uv = vUv * 2.0 - 1.0;
        uv.y *= -1.0;
        vec4 outCol = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));

        vec3 c0 = vec3(0.0, 0.0, 0.0);
        vec3 cDeep = vec3(11.0, 50.0, 33.0) / 255.0;
        vec3 cMid = vec3(19.0, 94.0, 61.0) / 255.0;
        vec3 cBright = vec3(26.0, 160.0, 109.0) / 255.0;
        vec3 cTeal = vec3(14.0, 214.0, 173.0) / 255.0;
        vec3 cLime = vec3(158.0, 255.0, 138.0) / 255.0;

        vec3 v = clamp(outCol.rgb, 0.0, 1.0);
        v = pow(v, vec3(0.72));
        float s = v.r + v.g + v.b;

        vec3 a = mix(cDeep, cTeal, clamp(v.b, 0.0, 1.0));
        vec3 b = mix(cMid, mix(cBright, cTeal, 0.55), clamp(v.g, 0.0, 1.0));
        vec3 c = mix(cBright, cLime, clamp(v.r, 0.0, 1.0));
        vec3 weighted = (v.r * c + v.g * b + v.b * a) / max(s, 0.001);

        float intensity = clamp(s / 2.05, 0.0, 1.0);
        intensity = pow(intensity, 0.58);
        intensity = smoothstep(0.02, 1.00, intensity);
        intensity = clamp(intensity * 1.08, 0.0, 1.0);

        float sweep = clamp(0.5 + 0.5 * sin(iTime * 0.08 + uv.x * 0.9 - uv.y * 0.55), 0.0, 1.0);
        vec3 sweepHue = mix(cTeal, cLime, sweep);
        weighted = mix(weighted, sweepHue, 0.10 * v.g);

        vec3 col = mix(c0, weighted, intensity);
        col = mix(col, cBright, 0.12 * pow(v.g, 2.0));
        col = mix(col, cTeal, 0.06 * pow(v.b, 2.0));
        col = mix(col, cLime, 0.05 * pow(v.r, 2.0));
    ...
    const resize = () => {
      const dprRaw = Number(window.devicePixelRatio || 1);
      const dpr = Number.isFinite(dprRaw) ? Math.max(1, Math.min(2, dprRaw)) : 1;
  ...
  return <canvas ref={canvasRef} className="landing-beams" aria-hidden="true" />;
}
